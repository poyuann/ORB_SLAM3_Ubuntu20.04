...
## Creat the folder to install ORB SLAM3 and dependencies
```shell=
cd ~
mkdir orb_slam && cd orb_slam
```

### Pangolin
Others version would be fine, v0.6 is comfirmed installable. 
```shell=
...
```shell=
cd ~/orb_slam
git clone https://github.com/poyuann/ORB_SLAM3_Ubuntu20.04.git
cd ORB_SLAM3_Ubuntu20.04
./build.sh
```
If it had root issue, run `chmod +x build.sh`

Incase, loacl can't get Pangolin run
```shell=
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:~/orb_slam/Pangolin/build
```

## ROS related 

```shell=
cd ~/orb_slam/ORB_SLAM3_Ubuntu20.04/ros
catkin_make
```

### Run with ros usb_cam or realsense package
```shell=
#roslaunch usb_cam usb_cam.launch
rosrun orb_slam orb_rosimg
```

### Run with cv read camera bus 
```shell=
rosrun orb_slam orb_cv
```
